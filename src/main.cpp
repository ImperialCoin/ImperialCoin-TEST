...
// Copyright (c) 2014 The ImperialCoin developers
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ImperialCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ImperialCoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("ImperialCoin miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    ...
    {
        printf("ImperialCoin miner terminated\n");
        throw;
    }
}
